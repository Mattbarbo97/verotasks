...
module.exports = function startTelegramOfficeWatcher(cfg, deps) {
  ...
  function fmtTaskTelegramTextHTML(task) {
    ...
    const officeState = escHtml(signalLabel(normalizeOfficeState(task.officeSignal)));
  ...
  async function trySendOrEditMasterMessage(task, htmlText) {
    ...
    if (masterMsgId) {
      await tgClient.editMessageText(masterChatId, masterMsgId, htmlText, { parse_mode: "HTML" });
      return { mode: "edit", messageId: masterMsgId };
    }

    const sent = await tgClient.sendMessage(masterChatId, htmlText, { parse_mode: "HTML" });
  ...
  async function tick() {
    ...
    try {
      ...
      for (const task of rows) {
        ...
        // se estiver pendente mas revision não está ok, ainda assim tentamos (ou marque falha)
        // aqui usamos needsDelivery como filtro extra:
        ...
        try {
          ...
          await trySendOrEditMasterMessage(fresh, htmlText);
